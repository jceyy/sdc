    ...
    for(int i = 1; i <= number_of_iterations; i++) {
        ...
        //   if(freeze_circle) {
            //    freeze_op->calculate_operator(iteration_vector[0], iteration_vector[1], iteration_vector[2], iteration_vector[3], iteration_vector[4]);
               // if(i == freeze_circle) {
               //     freeze_op->freeze_circle(iteration_vector[0], iteration_vector[1], iteration_vector[2], iteration_vector[3], iteration_vector[4], /*radius*/0.8);
             //  }
            //}
		...
		*/
		...
		/*else if(data.mode == 2) // speed
		{
			interactionMask->set_mask_parameters(data.posX, data.posY, data.radius, data.velX);
			interactionMask->masker(iteration_vector[1]);
			interactionMask->set_mask_parameters(data.posX, data.posY, data.radius, data.velY);
			interactionMask->masker(iteration_vector[2]);
		}*/
		else {} // do nothing
		//apply mask on 
...
// Y changes
 if(temp_circle) {
		addtemp_op->calculate_operator(iteration_vector[0]);
              ...
              if(i == temp_circle) {	//@ first iteration mask is prepared
                  addtemp_op->temp_rectangle(iteration_vector[0], /*radius*/0.1);
             }
           }
